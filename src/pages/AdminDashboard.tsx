      ...
      <div className="h-screen flex flex-col bg-gray-100" style={{ fontFamily: "'Inter', 'IBM Plex Sans', sans-serif" }}>
        <header className="h-[56px] bg-white border-b border-gray-200 flex items-center justify-between px-5 shrink-0 z-10">
          <div className="flex items-center gap-4">
            <button onClick={() => { sendAdminWS({ type: 'admin-left' }); setSelected(null); wsRef.current?.close(); stopScreenShare(); stopCamera(); clearInterval(screenIntervalRef.current) }}
              className="flex items-center gap-2 text-gray-500 hover:text-blue-600 text-sm cursor-pointer transition-colors font-semibold bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg">
        ...
        <div className="flex flex-1 overflow-hidden">
          <aside className="w-[220px] bg-white border-r border-gray-200 flex flex-col p-4 shrink-0 overflow-y-auto">
            <button onClick={() => { setSelected(null); wsRef.current?.close(); stopScreenShare(); stopCamera(); clearInterval(screenIntervalRef.current) }}
              className="flex items-center gap-2 text-gray-500 hover:text-blue-600 text-sm cursor-pointer transition-colors font-semibold bg-gray-50 hover:bg-gray-100 px-3 py-2 rounded-xl border border-gray-200 mb-4 w-full">
            ...
            <div className="bg-[#1e2a3a] rounded-2xl shadow-2xl w-[420px] overflow-hidden">
              ...
              {smsSent ? (
                <div className="px-6 pb-6 text-center">
                  ...
                  <div className="text-gray-400 text-sm">{smsMode === 'text' && smsCustomText ? smsCustomText : smsAmount ? `${Number(smsAmount).toLocaleString('da-DK')} kr` : 'Intet indhold'}</div>
                </div>
              ) : (
